...
document.addEventListener('DOMContentLoaded', function () {
    ...
    Swal.fire({
      title: '¡Bienvenido!',
      text: 'Iniciaste sesion en el administrador',
      icon: 'success',
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      document.getElementById("rollNo").addEventListener("input", function() {
        var horaSeleccionada = this.value.split(":")[0]; // Obtiene solo la hora
        if (horaSeleccionada < 8 || horaSeleccionada > 21) { // Cambia estos valores según tus necesidades
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Por favor, selecciona una hora entre las 8:00 y las 21:00.'
            });
            this.value = ''; // Limpia el valor si no está en el rango deseado
        }
    });